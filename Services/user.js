...
const joiValidLogin = joi.object({
    ...
    const exitsUser = await userSchema.userModel.findOne({ email: email })
    bcrypt.compare(password,exitsUser.password, function (err, isMatch) {
        if (err) res.send({status:400,message:"wrong password"})
